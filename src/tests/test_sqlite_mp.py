"""Test AseSqliteDB multi-process read and single-process write capabilities."""
...
from graphatoms.system._database.sqliteASE import AseSqliteDB
...
def _create_test_graph(name: str, idx: int) -> SysGraph:
    ...
    """
    atoms = g2[name]
    # Use smaller fmax values that are strictly below the 0.05 threshold
    # For idx up to 30, fmax = 0.01 + idx * 0.001 < 0.04
    return SysGraph.from_ase(
        atoms,
        energy=2.5 + idx * 0.1,
        fmax=0.01 + idx * 0.001,  # Ensure fmax < 0.05
        frequencies=np.array([51.0, 52.0, 53.0]) + idx * 2,  # Ensure all > 30
    )
...
def _reader_worker(
    ...
    try:
        db = AseSqliteDB(Path(db_path), append=True)
        for key in keys:
            if key in db:
                atoms = db[key]
...
def _periodic_writer_worker(
    ...
    pid = os.getpid()
    try:
        # Use different g2 names from initial data to avoid hash conflicts
        # Skip first 10 names to ensure uniqueness
        valid_names = g2.names[10 : 10 + num_graphs]
        for i, name in enumerate(valid_names):
            time.sleep(0.01)
            # Use smaller idx to ensure fmax = 0.01 + idx*0.001 < 0.05
            # idx < 40 ensures fmax < 0.05
            g = _create_test_graph(name, i + 30)  # 30+5=35 < 40, fmax=0.045
            db = AseSqliteDB(Path(db_path), append=True)
            success = db.add(g)
...
class TestAseSqliteDBMultiprocess:
    ...
    def test_multiple_readers_single_writer(self, test_db_path: Path):
        ...
        # Step 1: Create initial database with some data
        db = AseSqliteDB(test_db_path, append=False)
        initial_graphs = [
        ...
        db = AseSqliteDB(test_db_path, append=True)
        all_keys = list(db.keys())
        del db
        assert len(all_keys) == 10

        # Step 2: Start multiple reader processes
        num_readers = 4
        results_queue = multiprocessing.Queue()
        reader_processes = []

        # Distribute keys among readers
        ...
        # Step 3: Test single writer with concurrent readers
        results_queue = multiprocessing.Queue()

        # Start readers again (simulating concurrent read during write)
        reader_processes = []
        for i in range(num_readers):
            p = multiprocessing.Process(
                target=_reader_worker,
        ...
        # Verify all new graphs were written
        db = AseSqliteDB(test_db_path, append=True)
    ...
    def test_concurrent_read_consistency(self, test_db_path: Path):
        ...
        # Verify consistency: same key should always return same energy
    ...
    def test_write_during_read_race_condition(self, test_db_path: Path):
        ...
        # Start readers using module-level worker
        for _ in range(3):
            p = multiprocessing.Process(
        ...
        # Start writer using module-level worker
    ...
    def test_read_performance_scalability(self, test_db_path: Path):
        ...
        print(f"\n{'=' * 60}")
        print("READ PERFORMANCE SCALABILITY TEST")
        print(f"{'=' * 60}")
        ...
        # Detailed comparison between single and multi-process
        print()
        print(f"{'=' * 60}")
        ...
        if max_processes > 1:
            ...
            if scaling_efficiency < 50:
                print(
                    ...
                    "SQLite lock contention or I/O bottleneck)"
                )
    ...
    def test_write_performance(self, test_db_path: Path):
        ...
        for trial in range(num_trials):
            ...
            # Use different g2 names for each trial to avoid hash conflicts
            # Ensure idx < 40 to keep fmax < 0.05
    ...
    def test_mixed_workload_throughput(self, test_db_path: Path):
        ...
        print(f"\n{'=' * 60}")
        print("MIXED WORKLOAD THROUGHPUT TEST")
        print(f"{'=' * 60}")
        print(f"Initial entries: {len(read_keys)}")

        # Prepare write data (different from initial to avoid duplicates)
        # Use idx 15-24 to avoid conflicts with initial data (idx 0-14)
        write_graphs = [
            _create_test_graph(g2.names[i], i)
